...
export default class DynamicFormGenerator extends Component {
  ...
  inputChangedHandler = (elementType, placeholder, event, inputIdentifier) => {
    ...
    if(event.target.files){
      this.state.formFields[inputIdentifier].selectedFileObj = event.target.files[0];
    }
    ...
    this.setState(
      { formFields: updatedFormFields, formIsValid: formIsValid }
    );
  };
  ...
  render() {
    ...
    return <div className={styles.test}>{form}</div>;
  }
}
